...
<?php
  $sid=$_SESSION["SID"];
  $iid=$_SESSION["iid"];
  $ar=mysqli_query($conn,"SELECT * from nda WHERE iid='$iid' AND sid='$sid'");
  $res = mysqli_query($conn,"select * from ideas where sid='$sid'");
  $i = mysqli_fetch_assoc($res);
if($i["patent"]=='0' && mysqli_num_rows($ar)<=0)
{
    $_SESSION["idea_name"]=$i['idea_name'];
    header('refresh:0.01;url=http://localhost/AIRUP/form/nda.php');
}
?>
  <main id="main">
